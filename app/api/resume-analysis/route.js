...
export async function POST(req) {
  try {
    ...
    const { resumeContent, fileType } = body;

    // Handle both file upload and text content
    ...
    // Use text-based analysis for uploaded files
    const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });

    const analysisPrompt = fileType === 'resume' ? `You are a professional resume analyst. Analyze this resume carefully and provide structured feedback.

RESUME: 
${resumeContent}

Analyze based on:
1. Format & Structure - Clear sections, logical flow
2. Content Quality - Relevant achievements and skills
3. Keywords & ATS - Industry keywords, ATS optimization
4. Metrics & Numbers - Quantifiable results
5. Action Verbs - Strong, dynamic language
6. Grammar - Spelling and punctuation
7. Length - Appropriate detail level
8. Relevance - Tailored to target roles
...
Respond in this EXACT JSON format only (no markdown, no explanation):
{
  ...
  "suggestions": {
    "formatting": "specific formatting advice",
    "content": "specific content advice",
    "optimization": "ATS optimization advice"
  }
}` : `You are a professional CV analyst. Analyze this CV carefully and provide structured feedback.

CV: 
${resumeContent}

Analyze based on:
1. Academic Background - Clarity and credentials
2. Research & Publications - Relevant contributions
3. Professional Experience - Career progression
4. Skills - Technical and soft skills
5. Certifications - Relevant credentials
6. Formatting - Professional presentation
7. Impact - Clear achievements
8. Completeness - All relevant sections
...
Respond in this EXACT JSON format only (no markdown, no explanation):
{
  ...
  "improvements": ["improvement 1", "improvement 2", "improvement 3", "improvement 4"],
  "summary": "Brief overall assessment",
  "academicStrength": "Assessment of academic background",
  "suggestions": {
    "structure": "specific structural advice",
    "content": "specific content advice",
    "clarity": "clarity enhancement advice"
  }
}`;
    ...
    return Response.json({
      ...
      academicStrength: analysis.academicStrength || null
    });
...
// Keep file upload handler for future use
export async function POST_FILE(req) {
  try {
